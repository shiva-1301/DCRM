...
@login_required
def sos_page():
    return render_template("sos.html", user=current_user, sos_requests=get_user_sos_requests(current_user.id))
...
def create_sos():
    ...
    description = data.get("description", "")[:2000]  # cap length
    sos = create_sos_request(current_user.id, problem_type, description)
    return jsonify({"message": "SOS request created", "sos": {
        "id": str(sos["_id"]),
